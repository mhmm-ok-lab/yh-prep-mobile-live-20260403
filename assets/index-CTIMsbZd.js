...
if (score >= 5) {
    ...
    <div class="grid overview-grid">
        ...
        <div class="priority-strip">
          ${u.map(c=>`
                <button class="track-priority-btn ${ke(c.stage)} ${c.priority?"is-recommended":""}"
                  data-action="start-priority-track" data-track="${c.trackId}" data-mode="${c.priority?P.mode:"Drill"}">
                  <strong>${c.priority?"(R) ":""}${D(c.trackId)}</strong>
                  <span>${c.priority?P.mode:"Drill"} • ${fe(c.stage)} • ${c.avgScore}% • ${c.minutes} min</span>
                </button>
              `).join("")}
        </div>
      ...
      <section class="card span-6">
        <details>
          ...
          <ol class="list-clean">
            ${t.blocks.map(c=>`<li><span class="track-pill ${Z(c.trackId)}">${D(c.trackId)}</span> ${c.topic} (${c.minutes} min)</li>`).join("")}
          </ol>
    ...
    <div class="grid">
      ...
      ${e.map(r=>`
            <article class="card span-6">
              <p>
                <span class="track-pill ${Z(r.track_id)}">${D(r.track_id)}</span>
  ...
  `}function Pe(){const e=E.find(r=>r.id===W)||E[0],t=e.sections.reduce((r,i)=>r+i.question_ids.length,0);return`
    <div class="grid">
      ...
      ${e.sections.map(r=>`
            <section class="card span-6">
              <h3>${r.title}</h3>
              <p><span class="track-pill ${Z(r.track_id)}">${D(r.track_id)}</span></p>
  ...
  `}function De(){const e=Ct();return`
    <div class="grid">
      ...
      ${e.map(t=>`
            <article class="card span-6">
              <p>
                <span class="track-pill ${Z(t.track_id)}">${D(t.track_id)}</span>
  ...
  `}function Ee(){switch(L){case"overview":return wt();case"tracks":return Ie();case"bank":return Me();case"mock":return Pe();case"research":return Te();case"logic":return De();default:return wt()}}function k(){const e=U();z.innerHTML=`
    <div class="app">
      <header class="hero">
        <div class="hero-top">
          <div>
        ...
        <nav class="nav">
          ${Object.entries(Et).map(([t,r])=>`<button data-view="${t}" class="${L===t?"active":""}">${r}</button>`).join("")}
        </nav>
      </header>
