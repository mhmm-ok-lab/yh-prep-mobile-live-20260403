...
if (score >= 5) {
    ...
    <div class="grid overview-grid">
        ...
        <div class="priority-strip">
          ${u.map(c=>`
                <button class="track-priority-btn ${oe(c.stage)} ${c.priority?"is-recommended":""}"
                  data-action="start-priority-track" data-track="${c.trackId}" data-mode="${c.priority?I.mode:"Drill"}">
                  <strong>${c.priority?"(R) ":""}${P(c.trackId)}</strong>
                  <span>${c.priority?I.mode:"Drill"} • ${se(c.stage)} • ${c.avgScore}% • ${c.minutes} min</span>
                </button>
              `).join("")}
        </div>
        <p class="muted">${I.reason}</p>
      </section>
      ...
      <section class="card span-6">
        <details>
          ...
          <p class="muted">Aktiv användare: <strong>${w}</strong> • Storage: ${mt}</p>
        </details>
      </section>
      ...
      <section class="card span-6">
        <details>
          ...
          <p class="muted">Valt just nu: ${rt("sessionPreference",k.sessionPreference)} • ${rt("blocker",k.blocker)}</p>
        </details>
      </section>
      ...
      <section class="card span-6">
        <details>
          ...
          <ol class="list-clean">
            ${t.blocks.map(c=>`<li><span class="track-pill ${G(c.trackId)}">${P(c.trackId)}</span> ${c.topic} (${c.minutes} min)</li>`).join("")}
          </ol>
  ...
  `}function ke(){const e=xt(h,v),t=Array.from(new Set(h.map(r=>r.topic))).sort();return`
    <div class="grid">
      <section class="card span-12">
      ...
      ${e.map(r=>`
            <article class="card span-6">
              <p>
                <span class="track-pill ${G(r.track_id)}">${P(r.track_id)}</span>
  ...
  `}function fe(){const e=D.find(r=>r.id===X)||D[0],t=e.sections.reduce((r,i)=>r+i.question_ids.length,0);return`
    <div class="grid">
      ...
      ${e.sections.map(r=>`
            <section class="card span-6">
              <h3>${r.title}</h3>
              <p><span class="track-pill ${G(r.track_id)}">${P(r.track_id)}</span></p>
  ...
  `}function he(){const e=At();return`
    <div class="grid">
      ...
      ${e.map(t=>`
            <article class="card span-6">
              <p>
                <span class="track-pill ${G(t.track_id)}">${P(t.track_id)}</span>
  ...
  `}function ye(){switch(T){case"overview":return vt();case"tracks":return ge();case"bank":return ke();case"mock":return fe();case"research":return ve();case"logic":return he();default:return vt()}}function f(){const e=F();J.innerHTML=`
    <div class="app">
      <header class="hero">
        <div class="hero-top">
          <div>
          ...
          <div class="hero-actions">
            ...
            <details class="mini-menu user-menu">
              <summary class="user-avatar" title="Byt användare">${Wt(w)}</summary>
              <div class="mini-menu-body">
        ...
        <nav class="nav">
          ${Object.entries(It).map(([t,r])=>`<button data-view="${t}" class="${T===t?"active":""}">${r}</button>`).join("")}
        </nav>
      </header>
  ...
  `}J.addEventListener("click",e=>{const t=e.target,r=t.closest("button[data-view]");if(r){T=r.dataset.view,f();return}const i=t.closest("[data-action]");if(!i)return;const n=i.dataset.action;if(n){if(n==="jump-mock"){T="mock",f();return}if(n==="export-progress"){le();return}if(n==="start-today-drill"){const a=Nt(),s=F(),o=Number.parseInt(k.sessionPreference,10);S("Drill",a.map(p=>p.id),Number.isNaN(o)?s.totalMinutes:Math.max(30,Math.min(45,o)));return}if(n==="start-recommended"){const a=st(j());S(a.mode,a.questionIds,a.durationMinutes);return}if(n==="start-priority-track"){const a=i.dataset.track,s=i.dataset.mode||"Drill",o=st(j()),p=h.filter(g=>g.track_id===a).slice(0,10).map(g=>g.id),m=a===o.trackId?o.durationMinutes:25;S(s,p,m);return}if(n==="quick-tab"){const a=i.dataset.tab;a&&ce(a);return}if(n==="generate-ux-case"){Tt=ee(),f();return}if(n==="start-logic-drill"){const a=At();S("Drill",a.slice(0,12).map(s=>s.id),N(a));return}if(n==="start-track-drill"){const a=i.dataset.track,s=h.filter(o=>o.track_id===a).slice(0,8).map(o=>o.id);S("Drill",s,N(h.filter(o=>s.includes(o.id))));return}if(n==="start-track-learn"){const a=i.dataset.track,s=h.filter(o=>o.track_id===a).slice(0,6).map(o=>o.id);S("Lär",s,20);return}if(n==="open-python-course"){window.location.href="python-minikurs.html";return}if(n==="start-drill"){const a=xt(h,v);S("Drill",a.slice(0,12).map(s=>s.id),N(a));return}if(n==="start-mock"){const a=D.find(o=>o.id===X);if(!a)return;const s=a.sections.flatMap(o=>o.question_ids);S("Tidsprov",s,a.total_minutes,a.id);return}if(n==="save-profile"){re(k),A("Intervju sparad. Rekommenderad plan uppdaterad.");return}if(n==="generate-user-id"){q=zt(),f();return}if(n==="quick-switch-user"){const a=window.prompt("Ange user id (t.ex. martin, sara, klass-a):",w);if(a===null)return;W(a);return}if(n==="quick-switch-guest"){W("gäst");return}if(n==="switch-user-id"){W(q);return}if(l){if(n==="prev-q"){l.currentIndex=Math.max(0,l.currentIndex-1),E(l),f();return}if(n==="next-q"){l.currentIndex=Math.min(l.questionIds.length-1,l.currentIndex+1),E(l),f();return}if(n==="submit-session"){pt(!1);return}if(n==="cancel-session"){me();return}if(n==="answer"){const a=t,s=a.dataset.qid;if(!s)return;l.answers[s]=a.value,E(l),f()}}}});J.addEventListener("input",e=>{const t=e.target,r=t.closest("input[data-user-id='draft']");if(r){q=C(r.value);return}const i=t.closest("[data-profile]");if(i){const o=i.dataset.profile;o&&(t instanceof HTMLTextAreaElement||t instanceof HTMLSelectElement||t instanceof HTMLInputElement)&&(k={...k,[o]:t.value});return}const n=t.closest("textarea[data-action='answer-text']");if(n&&l){const o=n.dataset.qid;o&&(l.answers[o]=n.value,E(l));return}const a=t.closest("select[data-filter]");if(a){const o=a.dataset.filter;v={...v,[o]:a.value},f();return}const s=t.closest("select[data-mock-select='true']");s&&(X=s.value,f())});J.addEventListener("change",e=>{const r=e.target.closest("input[data-input='import-progress']");if(!r||!r.files||r.files.length===0)return;const[i]=r.files;de(i),r.value=""});window.addEventListener("online",()=>{lt=!0,f()});window.addEventListener("offline",()=>{lt=!1,f()});ue();l&&Date.now()>=l.endsAt?pt(!0):(ut(),f());
