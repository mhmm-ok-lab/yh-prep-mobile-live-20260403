...
if (score >= 5) {
  ...
  `}function Yt(){const e=["Logik/analys","Mattebas","Teststrategi","Programmeringsgrunder"];return`
    <div class="grid">
      ${J.map(t=>`
  ...
  `}function Wt(){const e=ct(k,f),t=Array.from(new Set(k.map(r=>r.topic))).sort();return`
    <div class="grid">
      <section class="card span-12">
        ...
        <div class="inline-controls">
          <select data-filter="trackId">
            ...
            ${J.map(r=>`<option value="${r.id}" ${f.trackId===r.id?"selected":""}>${r.name}</option>`).join("")}
          </select>

          <select data-filter="topic">
            <option value="all" ${f.topic==="all"?"selected":""}>Alla ämnen</option>
            ${t.map(r=>`<option value="${r}" ${f.topic===r?"selected":""}>${r}</option>`).join("")}
          </select>

          <select data-filter="difficulty">
            <option value="all" ${f.difficulty==="all"?"selected":""}>Alla nivåer</option>
            ${["Lätt","Medel","Svår"].map(r=>`<option value="${r}" ${f.difficulty===r?"selected":""}>${r}</option>`).join("")}
          </select>

          <select data-filter="sourceTier">
            <option value="all" ${f.sourceTier==="all"?"selected":""}>Alla källnivåer</option>
            ${["Officiell","Sekundär","Community"].map(r=>`<option value="${r}" ${f.sourceTier===r?"selected":""}>${r}</option>`).join("")}
          </select>
      ...
      ${e.map(r=>`
            <article class="card span-6">
              <p>
                <span class="track-pill ${R(r.track_id)}">${A(r.track_id)}</span>
  ...
  `}function Zt(){const e=I.find(r=>r.id===G)||I[0],t=e.sections.reduce((r,i)=>r+i.question_ids.length,0);return`
    <div class="grid">
      <section class="card span-12">
        <h3>Mockprov (tidsprov)</h3>
        <div class="inline-controls">
          <select data-mock-select="true">
            ${I.map(r=>`<option value="${r.id}" ${r.id===e.id?"selected":""}>${r.name}</option>`).join("")}
          </select>
      ...
      ${e.sections.map(r=>`
            <section class="card span-6">
              <h3>${r.title}</h3>
              <p><span class="track-pill ${R(r.track_id)}">${A(r.track_id)}</span></p>
  ...
  `}function te(){const e=L.filter(i=>i.source_tier==="Officiell").length,t=L.filter(i=>i.source_tier==="Community").length,r=L.filter(i=>i.source_tier==="Sekundär").length;return`
    <div class="grid">
      <section class="card span-12">
  ...
  `}function re(){if(!l)return"";const e=kt(l),t=e[l.currentIndex];if(!t)return"";const r=l.endsAt-Date.now(),i=Object.keys(l.answers).length,n=l.answers[t.id]||"";return`
    <section class="card session">
      <h3>Aktiv ${l.mode}</h3>
      <p class="session-timer ${r<5*6e4?"warning":""}">Tid kvar: ${Vt(r)}</p>
  ...
  `}function ie(){if(!m)return"";const e=m.sectionResults.length===0?"":`
  ...
  `}function ne(){switch(y){case"overview":return nt();case"tracks":return Yt();case"bank":return Wt();case"mock":return Zt();case"research":return te();case"logic":return ee();default:return nt()}}function u(){const e=F();q.innerHTML=`
    <div class="app">
      <header class="hero">
        <div class="hero-top">
          <div>
        ...
        <nav class="nav">
          ${Object.entries(Et).map(([t,r])=>`<button data-view="${t}" class="${y===t?"active":""}">${r}</button>`).join("")}
        </nav>
      </header>
  ...
  `}q.addEventListener("click",e=>{const t=e.target,r=t.closest("button[data-view]");if(r){y=r.dataset.view,u();return}const i=t.closest("[data-action]");if(!i)return;const n=i.dataset.action;if(n){if(n==="jump-mock"){y="mock",u();return}if(n==="export-progress"){Qt();return}if(n==="start-today-drill"){const a=_t(),o=F(),s=Number.parseInt(p.sessionPreference,10);b("Drill",a.map(c=>c.id),Number.isNaN(s)?o.totalMinutes:Math.max(30,Math.min(45,s)));return}if(n==="start-priority-learn"){const a=W(p.targetPriority),o=k.filter(s=>s.track_id===a).slice(0,6);b("Lär",o.map(s=>s.id),20);return}if(n==="quick-tab"){const a=i.dataset.tab;a&&Ht(a);return}if(n==="generate-ux-case"){ft=Rt(),u();return}if(n==="start-logic-drill"){const a=vt();b("Drill",a.slice(0,12).map(o=>o.id),M(a));return}if(n==="start-track-drill"){const a=i.dataset.track,o=k.filter(s=>s.track_id===a).slice(0,8).map(s=>s.id);b("Drill",o,M(k.filter(s=>o.includes(s.id))));return}if(n==="start-track-learn"){const a=i.dataset.track,o=k.filter(s=>s.track_id===a).slice(0,6).map(s=>s.id);b("Lär",o,20);return}if(n==="open-python-course"){window.open("python-minikurs.html","_blank","noopener,noreferrer");return}if(n==="start-drill"){const a=ct(k,f);b("Drill",a.slice(0,12).map(o=>o.id),M(a));return}if(n==="start-mock"){const a=I.find(s=>s.id===G);if(!a)return;const o=a.sections.flatMap(s=>s.question_ids);b("Tidsprov",o,a.total_minutes,a.id);return}if(n==="save-profile"){Bt(p),x("Intervju sparad. Rekommenderad plan uppdaterad.");return}if(n==="generate-user-id"){T=Ft(),u();return}if(n==="switch-user-id"){const a=Ct(T);S=a,T=a,ut(a),l=V(),p=ht(a),m=null,v&&(window.clearInterval(v),v=null),Z(),x(`Bytt till användare: ${a}`);return}if(l){if(n==="prev-q"){l.currentIndex=Math.max(0,l.currentIndex-1),P(l),u();return}if(n==="next-q"){l.currentIndex=Math.min(l.questionIds.length-1,l.currentIndex+1),P(l),u();return}if(n==="submit-session"){tt(!1);return}if(n==="cancel-session"){Gt();return}if(n==="answer"){const a=t,o=a.dataset.qid;if(!o)return;l.answers[o]=a.value,P(l),u()}}}});q.addEventListener("input",e=>{const t=e.target,r=t.closest("input[data-user-id='draft']");if(r){T=U(r.value);return}const i=t.closest("[data-profile]");if(i){const s=i.dataset.profile;s&&(t instanceof HTMLTextAreaElement||t instanceof HTMLSelectElement||t instanceof HTMLInputElement)&&(p={...p,[s]:t.value});return}const n=t.closest("textarea[data-action='answer-text']");if(n&&l){const s=n.dataset.qid;s&&(l.answers[s]=n.value,P(l));return}const a=t.closest("select[data-filter]");if(a){const s=a.dataset.filter;f={...f,[s]:a.value},u();return}const o=t.closest("select[data-mock-select='true']");o&&(G=o.value,u())});q.addEventListener("change",e=>{const r=e.target.closest("input[data-input='import-progress']");if(!r||!r.files||r.files.length===0)return;const[i]=r.files;Jt(i),r.value=""});window.addEventListener("online",()=>{Y=!0,u()});window.addEventListener("offline",()=>{Y=!1,u()});Xt();l&&Date.now()>=l.endsAt?tt(!0):(Z(),u());
