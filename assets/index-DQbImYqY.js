...
if (score >= 5) {
    ...
    <div class="grid overview-grid">
        ...
        <div class="priority-strip">
          ${d.map(s=>`
                <button class="track-priority-btn ${ge(s.stage)} ${s.priority?"is-recommended":""}"
                  ...
                  <strong>${s.priority?"(R) ":""}${M(s.trackId)}</strong>
      ...
      <section class="card span-6">
        <details>
          ...
          <ol class="list-clean">
            ${t.blocks.map(s=>`<li><span class="track-pill ${W(s.trackId)}">${M(s.trackId)}</span> ${s.topic} (${s.minutes} min)</li>`).join("")}
          </ol>
  ...
  `}function $e(){const e=Mt(h,v),t=Array.from(new Set(h.map(r=>r.topic))).sort();return`
    <div class="grid">
      <section class="card span-12">
      ...
      ${e.map(r=>`
            <article class="card span-6">
              <p>
                <span class="track-pill ${W(r.track_id)}">${M(r.track_id)}</span>
  ...
  `}function Ie(){const e=A.find(r=>r.id===z)||A[0],t=e.sections.reduce((r,i)=>r+i.question_ids.length,0);return`
    <div class="grid">
      <section class="card span-12">
      ...
      ${e.sections.map(r=>`
            <section class="card span-6">
              <h3>${r.title}</h3>
              <p><span class="track-pill ${W(r.track_id)}">${M(r.track_id)}</span></p>
  ...
  `}function Me(){const e=U.filter(i=>i.source_tier==="Officiell").length,t=U.filter(i=>i.source_tier==="Community").length,r=U.filter(i=>i.source_tier==="Sekundär").length;return`
    <div class="grid">
      <section class="card span-12">
  ...
  `}function Pe(){const e=jt();return`
    <div class="grid">
      ...
      ${e.map(t=>`
            <article class="card span-6">
              <p>
                <span class="track-pill ${W(t.track_id)}">${M(t.track_id)}</span>
  ...
  `}function Le(){switch(P){case"overview":return xt();case"tracks":return Se();case"bank":return $e();case"mock":return Ie();case"research":return Me();case"logic":return Pe();default:return xt()}}function k(){const e=R();Y.innerHTML=`
    <div class="app">
      <header class="hero">
        <div class="hero-top">
          <div>
          ...
          <div class="hero-actions">
            ...
            <details class="mini-menu context-menu">
              <summary>Läge: ${lt[P]}</summary>
              <div class="mini-menu-body">
                ${Object.entries(lt).map(([t,r])=>`<button class="secondary ${P===t?"active":""}" data-view="${t}">${r}</button>`).join("")}
  ...
  `}Y.addEventListener("click",e=>{const t=e.target,r=t.closest("button[data-view]");if(r){P=r.dataset.view,k();return}const i=t.closest("[data-action]");if(!i)return;const n=i.dataset.action;if(n){if(n==="export-progress"){ke();return}if(n==="start-today-drill"){const a=Ft(),o=R(),c=Number.parseInt(f.sessionPreference,10);$("Drill",a.map(u=>u.id),Number.isNaN(c)?o.totalMinutes:Math.max(30,Math.min(45,c)));return}if(n==="start-priority-track"){const a=i.dataset.track,o=i.dataset.mode||"Drill",c=Ct(G()),u=h.filter(m=>m.track_id===a).slice(0,10).map(m=>m.id),p=a===c.trackId?c.durationMinutes:25;$(o,u,p);return}if(n==="quick-tab"){const a=i.dataset.tab;a&&fe(a);return}if(n==="generate-ux-case"){Nt=ce(),k();return}if(n==="start-logic-drill"){const a=jt();$("Drill",a.slice(0,12).map(o=>o.id),C(a));return}if(n==="start-track-drill"){const a=i.dataset.track,o=h.filter(c=>c.track_id===a).slice(0,8).map(c=>c.id);$("Drill",o,C(h.filter(c=>o.includes(c.id))));return}if(n==="start-track-learn"){const a=i.dataset.track,o=h.filter(c=>c.track_id===a).slice(0,6).map(c=>c.id);$("Lär",o,20);return}if(n==="open-python-course"){window.location.href="python-minikurs.html";return}if(n==="start-drill"){const a=Mt(h,v);$("Drill",a.slice(0,12).map(o=>o.id),C(a));return}if(n==="start-mock"){const a=A.find(c=>c.id===z);if(!a)return;const o=a.sections.flatMap(c=>c.question_ids);$("Tidsprov",o,a.total_minutes,a.id);return}if(n==="save-profile"){le(f),D("Intervju sparad. Rekommenderad plan uppdaterad.");return}if(n==="quick-switch-user"){const a=window.prompt("Ange user id (t.ex. martin, sara, klass-a):",T);if(a===null)return;tt(a);return}if(n==="quick-switch-guest"){tt("gäst");return}if(n==="quick-generate-user"){const a=re();tt(a);return}if(l){if(n==="prev-q"){l.currentIndex=Math.max(0,l.currentIndex-1),E(l),k();return}if(n==="next-q"){l.currentIndex=Math.min(l.questionIds.length-1,l.currentIndex+1),E(l),k();return}if(n==="submit-session"){vt(!1);return}if(n==="cancel-session"){we();return}if(n==="answer"){const a=t,o=a.dataset.qid;if(!o)return;l.answers[o]=a.value,E(l),k()}}}});Y.addEventListener("input",e=>{const t=e.target,r=t.closest("[data-profile]");if(r){const o=r.dataset.profile;o&&(t instanceof HTMLTextAreaElement||t instanceof HTMLSelectElement||t instanceof HTMLInputElement)&&(f={...f,[o]:t.value});return}const i=t.closest("textarea[data-action='answer-text']");if(i&&l){const o=i.dataset.qid;o&&(l.answers[o]=i.value,E(l));return}const n=t.closest("select[data-filter]");if(n){const o=n.dataset.filter;v={...v,[o]:n.value},k();return}const a=t.closest("select[data-mock-select='true']");a&&(z=a.value,k())});Y.addEventListener("change",e=>{const r=e.target.closest("input[data-input='import-progress']");if(!r||!r.files||r.files.length===0)return;const[i]=r.files;ve(i),r.value=""});window.addEventListener("online",()=>{gt=!0,k()});window.addEventListener("offline",()=>{gt=!1,k()});he();ye();l&&Date.now()>=l.endsAt?vt(!0):(kt(),k());
